 ...
 */
class ObsiaSignup extends HTMLElement {
  #boundGoogleHandler = null;

  /**
   * @description Constructs the ObsiaSignup instance.
   * @returns {void}
   */
  constructor() {
    super();
  ...
  connectedCallback() {
    ...
    this.innerHTML = /*html*/ `
      <div class="w-[500px] bg-slate-950 p-8 flex flex-col rounded-3xl border border-slate-700">
        ...
        <obsia-google-signin></obsia-google-signin>

      </div>
    `;

    this.querySelector("#signupForm").addEventListener("submit", this.#handleSignup.bind(this));
    this.querySelector("obsia-google-signin").addEventListener(
  ...
  disconnectedCallback() {
    ...
    this.querySelector("obsia-google-signin").removeEventListener(
      "google-signin-result",
      this.#handleGoogleSigninResult.bind(this),
    );
  }
  ...
  async #handleSignup(event) {
    ...
    signupButton.innerHTML = /*html*/ `
      <img src=\"${obsia_vars.plugin_url}assets/icons/loader.svg\" class=\"animate-spin\" />
    `;
...
customElements.define("obsia-signup", ObsiaSignup);
