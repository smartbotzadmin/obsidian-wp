/**
 * @class ObsiaPicturesSearchBar
 * @augments HTMLElement
 * @description Web component for the image search bar on the pictures page.
 */
class ObsiaPicturesSearchBar extends HTMLElement {
  searchInput = null;
  clearButton = null;

  /**
   * @description Constructs the ObsiaPicturesSearchBar instance.
   * @returns {void}
   */
  constructor() {
    super();
    ...
    this.innerHTML = `
            ...
            <button id="clearSearchButton" class="absolute right-3 cursor-pointer hidden">
                <img src="/wp-content/plugins/obsia/assets/icons/x.svg" class="h-5 w-5" />
            </button>
        `;
  }
  ...
  #handleSearchKeyDown(event) {
    if (event.key === "Enter") {
      ...
      window.dispatchEvent(
        new CustomEvent("search-triggered", {
          detail: { query: query },
        }),
      );
    }
...
customElements.define("obsia-pictures-search-bar", ObsiaPicturesSearchBar);
