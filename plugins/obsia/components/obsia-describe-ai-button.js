/**
 * @class ObsiaDescribeAiButton
 * @augments HTMLElement
 * @description Web component for the "Write Using AI" button.
 */
class ObsiaDescribeAiButton extends HTMLElement {
  /**
   * @description Constructs the ObsiaDescribeAiButton instance.
   * @returns {void}
   */
  constructor() {
    super();
    this.innerHTML = `
            <button id="writeAiButton" class="flex items-center gap-2 text-slate-100 hover:text-cyan-400 font-semibold cursor-pointer p-2 rounded-xl transition-all duration-300 outline outline-transparent hover:outline-cyan-500 hover:outline-1 focus:outline-cyan-500 focus:outline-2">
                <img src="${obsia_vars.plugin_url}assets/icons/wand-sparkles-dark.svg"/>
                Write Using AI
            </button>
        `;
  ...
  async #handleWriteAiClick() {
    ...
    try {
      const payload = window.obsiaSessionManager.getPayload();
      ...
      window.dispatchEvent(
        new CustomEvent("ai-text-generated", {
...
customElements.define("obsia-describe-ai-button", ObsiaDescribeAiButton);
