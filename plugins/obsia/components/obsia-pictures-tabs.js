...
class ObsiaPicturesTabs extends HTMLElement {
  ...
  /**
   * @description Constructs the ObsiaPicturesTabs instance.
   * @returns {void}
   */
  constructor() {
    super();
    ...
    this.innerHTML = `
            ...
            <div class="relative flex text-left">
                <div class="inline-flex justify-center w-full rounded-xl shadow-sm px-4 py-2 bg-slate-900 text-[14px] font-semibold text-slate-300 hover:bg-slate-800 transition-all duration-300 ease-in-out outline outline-0 outline-transparent hover:outline-1 hover:outline-cyan-500 focus:outline-2 focus:outline-cyan-500 h-11 items-center cursor-pointer" id="orientation-menu-button" aria-expanded="true" aria-haspopup="true">
                    ...
                    <img src="/wp-content/plugins/obsia/assets/icons/chevron-down.svg" class="ml-2 -mr-1 h-4 w-4 transition-transform duration-300 ease-in-out" alt="Dropdown icon">
                </div>
  ...
  connectedCallback() {
    ...
    this.orientationMenuButton.addEventListener(
      "click",
      this.#handleOrientationMenuClick.bind(this),
    );
  ...
  disconnectedCallback() {
    ...
    if (this.orientationMenuButton) {
      this.orientationMenuButton.removeEventListener(
        "click",
        this.#handleOrientationMenuClick.bind(this),
      );
    }
  ...
  #handleOutsideClick(event) {
    ...
    if (
      !this.orientationMenuButton.contains(event.target) &&
      !this.orientationDropdown.contains(event.target)
    ) {
...
customElements.define("obsia-pictures-tabs", ObsiaPicturesTabs);
