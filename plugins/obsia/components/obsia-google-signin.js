/**
 * @class ObsiaGoogleSignin
 * @augments HTMLElement
 * @description Web component for the Google Sign-in button.
 */
class ObsiaGoogleSignin extends HTMLElement {
  #channel = null;

  /**
   * @description Constructs the ObsiaGoogleSignin instance.
   * @returns {void}
   */
  constructor() {
    super();
  ...
  connectedCallback() {
    this.innerHTML = /*html*/ `
      <button id="googleSigninButton" class="w-full h-11 bg-transparent hover:bg-slate-900 border border-slate-700 text-white font-semibold text-md py-2 px-4 flex justify-center items-center gap-2 rounded-xl focus:outline-none focus:shadow-outline transition-all duration-300 ease-in-out cursor-pointer">
        <img src="/wp-content/plugins/obsia/assets/icons/google-icon.svg" class="w-5 h-5" />
        Sign in with Google
      </button>
    `;
...
customElements.define("obsia-google-signin", ObsiaGoogleSignin);
