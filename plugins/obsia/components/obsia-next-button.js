...
class ObsiaNextButton extends HTMLElement {
  ...
  /**
   * @description Constructs the ObsiaNextButton instance.
   * @returns {void}
   */
  constructor() {
    super();
    ...
    this.innerHTML = /*html*/ `
            ...
            <button id="nextButton" class="flex flex-row justify-center items-center gap-2 bg-cyan-600 hover:bg-cyan-500 p-4 h-11 rounded-xl flex items-center cursor-pointer transition-colors duration-300">
                ...
                <img src="/wp-content/plugins/obsia/assets/icons/arrow-right.svg"/>
            </button>
  ...
  #handleButtonClick() {
    ...
    const statusPayload = window.obsiaSessionManager.checkPayloadStatus();
...
customElements.define("obsia-next-button", ObsiaNextButton);
